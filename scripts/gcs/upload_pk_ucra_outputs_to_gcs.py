"""Upload Pakistan UCRA outputs (13 per-city folders + stats + plots) to
gs://crp-city-scan/Ucra-Pakistan-cities/, preserving folder structure.
Parallel to the Bangladesh upload. ADC (azizlums) auth; resumable; retried.
"""
import os
import sys
...
DEST_PREFIX = "Ucra-Pakistan-cities"
HERE = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.dirname(HERE)
PK = os.path.join(PROJECT_ROOT, "Pakistan")

CITIES = [
    "Ahmadpur East", "Arifwala", "Chishtian", "Haroonabad", "Jampur", "Jatoi",
    "Khanpur", "Mandi Bahauddin", "Pakpattan", "Rajanpur", "Sadiqabad",
    "Shujabad", "Sumandri",
]
SRC_DIRS = CITIES + ["stats", "plots"]
...
tasks = []
for sub in SRC_DIRS:
    root_dir = os.path.join(PK, sub)
    if not os.path.isdir(root_dir):
        print(f"  WARN missing: {sub}", flush=True)
        continue
    for root, _dirs, files in os.walk(root_dir):
        for fn in files:
            lp = os.path.join(root, fn)
            rel = os.path.relpath(lp, PK).replace("\\", "/")
...
print(f"Total files: {total} | already present: {skipped} | "
      f"to upload: {len(to_upload)} ({sum(t[2] for t in to_upload)/1e6:.0f} MB)\n", flush=True)
...
if failed:
    ...
    sys.exit(1)
print(f"\nAll Pakistan UCRA outputs present under gs://{BUCKET}/{DEST_PREFIX}/", flush=True)
